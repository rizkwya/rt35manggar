            ...
            <div className="space-y-1.5">
              <div className="flex items-center justify-between">
                <label className="block text-xs font-black uppercase tracking-wider text-slate-500">
                  Kategori / Badge <span className="text-[10px] text-slate-400 font-bold lowercase">(opsional)</span>
                </label>
              </div>
              <input
                type="text"
                placeholder="Contoh: HUT RI, Lomba Rakyat, Sosial (pisahkan koma)"
                value={kegiatanBadge}
              ...
              <p className="text-[10px] text-slate-400 font-semibold">
                Tips: Gunakan kata singkat atau pisahkan dengan koma jika lebih dari satu tag.
              </p>
            </div>
            ...
            <div className="space-y-4">
              <div className="space-y-3">
                ...
                {paginatedItems.map((item, idx) => {
                  ...
                  return (
                    <div key={originalIdx} className="p-4 sm:p-5 rounded-2xl bg-white border border-slate-200 flex flex-col sm:flex-row items-start justify-between gap-4 shadow-sm hover:border-slate-300 transition-all overflow-hidden">
                      <div className="flex items-start space-x-3.5 sm:space-x-4 min-w-0 flex-1 w-full">
                        {item.image_url ? (
                          <img src={item.image_url} alt={item.title} className="w-14 h-14 sm:w-16 sm:h-16 rounded-xl object-cover shrink-0 border border-slate-100 shadow-xs" />
                        ) : (
                          <div className="w-14 h-14 sm:w-16 sm:h-16 rounded-xl bg-slate-100 flex items-center justify-center shrink-0 text-slate-400">
                        ...
                        <div className="min-w-0 flex-1 space-y-1.5">
                          {badges.length > 0 && (
                            <div className="flex items-center gap-1.5 flex-wrap">
                              {badges.map((badgeText: string, bIdx: number) => (
                                <span 
                                  key={bIdx} 
                                  className="px-2 py-0.5 text-[9px] font-extrabold bg-[#85A389]/15 text-[#38663e] rounded-md border border-[#85A389]/30 uppercase tracking-wide break-words max-w-full inline-block"
                                >
                                  {badgeText}
                                </span>
                              ))}
                          ...
                          <h4 className="text-sm font-black text-slate-900 leading-snug break-words line-clamp-2">
                            {item.title}
                          </h4>
                      ...
                      <div className="flex items-center space-x-2 w-full sm:w-auto shrink-0 justify-end border-t sm:border-t-0 pt-3 sm:pt-0 border-slate-100 self-stretch sm:self-center">
                        <button
                          type="button"
                          onClick={() => {
                          ...
                          className="px-3.5 py-2 rounded-xl bg-slate-50 hover:bg-slate-100 text-slate-700 border border-slate-200 text-xs font-bold flex items-center justify-center gap-1.5 shadow-xs transition-all flex-1 sm:flex-initial"
                          title="Edit Kegiatan"
                        >
                        ...
                        <button
                          ...
                          className="px-3.5 py-2 rounded-xl bg-rose-50 hover:bg-rose-100 text-rose-600 border border-rose-200 text-xs font-bold flex items-center justify-center gap-1.5 shadow-xs transition-all flex-1 sm:flex-initial"
